...
# デフォルト類似度閾値
# ADR-002修正: 0.35 → 0.40 → 0.72 に引き上げ
#   0.40では multilingual-e5-large の日本語分布で無関係質問でも 0.77 前後の
#   類似度が出て、ハルシネーション抑制が不十分だった（2026-04-18 実測）。
#   当初 0.72 としたが、再テストで無関係質問の類似度が 0.77〜0.79 まで到達することを
#   確認し、0.80 に再調整（2026-04-18）。資料関連の最低類似度 0.86 より十分マージンあり。
#   ユーザー調整UIは廃止し、開発側で最適値を固定リリースする方針。
